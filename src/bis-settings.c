...
#include "bis-settings-private.h"
...
struct _BisSettings
{
  ...
  BisSystemColorScheme color_scheme_override;
  gboolean high_contrast_override;
};

G_DEFINE_FINAL_TYPE (BisSettings, bis_settings, G_TYPE_OBJECT);
...
static BisSettings *default_instance;

static void
set_color_scheme (BisSettings          *self,
                  BisSystemColorScheme  color_scheme)
{
...
static void
set_high_contrast (BisSettings *self,
                   gboolean     high_contrast)
{
...
static void
init_debug (BisSettings *self)
{
  const char *env = g_getenv ("BIS_DEBUG_HIGH_CONTRAST");
  if (env && *env) {
    if (!g_strcmp0 (env, "1")) {
      ...
      g_warning ("Invalid value for BIS_DEBUG_HIGH_CONTRAST: %s (Expected 0 or 1)", env);
    }
  }

  env = g_getenv ("BIS_DEBUG_COLOR_SCHEME");
  if (env) {
    if (!g_strcmp0 (env, "default")) {
      self->has_color_scheme = TRUE;
      self->color_scheme = BIS_SYSTEM_COLOR_SCHEME_DEFAULT;
    } else if (!g_strcmp0 (env, "prefer-dark")) {
      self->has_color_scheme = TRUE;
      self->color_scheme = BIS_SYSTEM_COLOR_SCHEME_PREFER_DARK;
    } else if (!g_strcmp0 (env, "prefer-light")) {
      self->has_color_scheme = TRUE;
      self->color_scheme = BIS_SYSTEM_COLOR_SCHEME_PREFER_LIGHT;
    } else {
...
{
  const char *disable_portal = g_getenv ("BIS_DISABLE_PORTAL");

  return disable_portal && disable_portal[0] == '1';
}

static gboolean
read_portal_setting (BisSettings  *self,
                     const char   *schema,
                     const char   *name,
                     const char   *type,
                     GVariant    **out)
...
get_gnome_color_scheme (GVariant *variant)
{
  ...
  if (!g_strcmp0 (str, "default"))
    return BIS_SYSTEM_COLOR_SCHEME_DEFAULT;

  if (!g_strcmp0 (str, "prefer-dark"))
    return BIS_SYSTEM_COLOR_SCHEME_PREFER_DARK;

  if (!g_strcmp0 (str, "prefer-light"))
    return BIS_SYSTEM_COLOR_SCHEME_PREFER_LIGHT;
...
{
  BisSettings *settings;
}
@end

@implementation ThemeChangedObserver

-(instancetype)initWithSettings:(BisSettings *)_settings
{
  [self init];
...
static void
init_nsapp_observer (BisSettings *settings)
{
...
static void
display_setting_changed_cb (BisSettings *self,
                            const char  *setting,
                            GdkDisplay  *display)
{
...
static void
bis_settings_constructed (GObject *object)
{
  BisSettings *self = BIS_SETTINGS (object);

  G_OBJECT_CLASS (bis_settings_parent_class)->constructed (object);
...
static void
bis_settings_dispose (GObject *object)
{
  BisSettings *self = BIS_SETTINGS (object);
  ...
  G_OBJECT_CLASS (bis_settings_parent_class)->dispose (object);
}

static void
bis_settings_get_property (GObject    *object,
                           guint       prop_id,
                           GValue     *value,
                           GParamSpec *pspec)
{
  ...
  case PROP_SYSTEM_SUPPORTS_COLOR_SCHEMES:
    g_value_set_boolean (value, bis_settings_get_system_supports_color_schemes (self));
    break;

  case PROP_COLOR_SCHEME:
    g_value_set_enum (value, bis_settings_get_color_scheme (self));
    break;

  case PROP_HIGH_CONTRAST:
    g_value_set_boolean (value, bis_settings_get_high_contrast (self));
    break;
...
static void
bis_settings_class_init (BisSettingsClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = bis_settings_constructed;
  object_class->dispose = bis_settings_dispose;
  object_class->get_property = bis_settings_get_property;
...
static void
bis_settings_init (BisSettings *self)
{
...
BisSettings *
bis_settings_get_default (void)
{
  if (!default_instance)
    default_instance = g_object_new (BIS_TYPE_SETTINGS, NULL);

  return default_instance;
}

gboolean
bis_settings_get_system_supports_color_schemes (BisSettings *self)
{
  g_return_val_if_fail (BIS_IS_SETTINGS (self), FALSE);
...
bis_settings_get_color_scheme (BisSettings *self)
{
  g_return_val_if_fail (BIS_IS_SETTINGS (self), BIS_SYSTEM_COLOR_SCHEME_DEFAULT);
...
gboolean
bis_settings_get_high_contrast (BisSettings *self)
{
  g_return_val_if_fail (BIS_IS_SETTINGS (self), FALSE);
...
void
bis_settings_start_override (BisSettings *self)
{
  g_return_if_fail (BIS_IS_SETTINGS (self));
...
void
bis_settings_end_override (BisSettings *self)
{
  ...
  self->color_scheme_override = BIS_SYSTEM_COLOR_SCHEME_DEFAULT;
...
void
bis_settings_override_system_supports_color_schemes (BisSettings *self,
                                                     gboolean     system_supports_color_schemes)
{
  ...
  if (!system_supports_color_schemes)
    bis_settings_override_color_scheme (self, BIS_SYSTEM_COLOR_SCHEME_DEFAULT);
...
void
bis_settings_override_color_scheme (BisSettings          *self,
                                    BisSystemColorScheme  color_scheme)
{
  g_return_if_fail (BIS_IS_SETTINGS (self));
  g_return_if_fail (self->override);
...
void
bis_settings_override_high_contrast (BisSettings *self,
                                     gboolean     high_contrast)
{
  g_return_if_fail (BIS_IS_SETTINGS (self));
  g_return_if_fail (self->override);
